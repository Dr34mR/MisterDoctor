...
namespace MisterDoctor.Plugins
{
    ...
    {
        private readonly List<char> _delims = new() { ' ', '?', '!', ',', '.', '\"', '@' };
        ...
        public MessageParts CreateCopy()
        {
            var returnParts = new MessageParts();
            foreach (var message in this)
            {
                returnParts.Add(new MessagePart
                {
                    IsNoun = message.IsNoun,
